...
- **Node**: the SDK is ESM (`type: module`) with zero runtime dependencies and one peer (`@openfeature/server-sdk`). `@fireweaveai/sdk/posthog` was removed in v3 — a `Cannot find module` on that subpath means you are on v2 code with a v3 package ([migration](migration.md#from-fireweaveaisdk-v2-to-v3-node)). Integers beyond 2^53−1 are not lossless (single `number` resolver). On Deno, reading `FW_API_URL`/`FW_PROJECT_API_KEY` from the environment needs `--allow-env`; passing them explicitly needs no env permission ([runtimes.md](runtimes.md)).
