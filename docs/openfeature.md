...
Whether an evaluation may emit a `$feature_flag_called`-style exposure event is controlled by adapter configuration, not by the OpenFeature call site. Current defaults are conservative: the Node and Python PostHog adapters use side-effect-free snapshot reads (vendor exposure events disabled; exposures flow through the explicit `exposures` API); the Go PostHog adapter emits vendor exposure events only when `SendExposureEvents: true`. See [extensions.md](extensions.md#exposures) and [posthog.md](posthog.md).
