...
def _deep_copy_json(value: Any, seen: Optional[set] = None) -> Any:
    ...
    if isinstance(value, dict):
        ...
        seen.add(marker)
        try:
            return {k: _deep_copy_json(v, seen) for k, v in value.items()}
        finally:
            seen.discard(marker)
    if isinstance(value, list):
        ...
        seen.add(marker)
        try:
            return [_deep_copy_json(v, seen) for v in value]
        finally:
            seen.discard(marker)
    return value
