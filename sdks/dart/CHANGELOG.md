...
Initial package. Control-point evaluation and target registration (spec v0.1.0) — the
two v1 capabilities — with synchronous reads over a prefetched cache
...
no `flutter` SDK dependency. Version is in lockstep with the other Fireweave SDK
manifests.
