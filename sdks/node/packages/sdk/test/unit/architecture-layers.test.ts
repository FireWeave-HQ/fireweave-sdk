 ...
 *    wiring along;
 *  - `src/application/` does not reach into `infrastructure/` except through
 *    two sanctioned seams (controller ruling, Task 4 fix round):
 *    `mode.ts`, the composition root (the plan places "mode" in
 *    `application/` and its whole job is adapter selection, so its concrete
 *    `infrastructure/adapters/*` imports are exempt wholesale), and
 *    `runtime.ts`'s single import of `assertHostAllowed` from
 *    `infrastructure/hosts.ts` (a pure function; the plan text places
 *    `hosts.ts` in `infrastructure/` regardless of its purity), which is
 *    allowlisted explicitly rather than exempted wholesale.
 */
...
/**
 * `mode.ts` is the SANCTIONED composition root (controller ruling, Task 4
 * fix round): the plan places "mode" in `application/` and its defined job
 * is adapter selection, so its concrete `infrastructure/adapters/*` imports
 * are expected and exempt wholesale — it is skipped entirely below rather
 * than allowlisted specifier-by-specifier.
 */
const APPLICATION_COMPOSITION_ROOT = 'mode.ts';

/**
 * Every other `application/` file's `infrastructure/` imports must appear
 * here, keyed by its path relative to `application/`. Currently exactly one
 * pair: `runtime.ts` calling `assertHostAllowed`, a pure function that the
 * plan text places in `infrastructure/hosts.ts` despite its purity
 * (controller ruling, Task 4 fix round). Anything else reaching into
 * `infrastructure/` from outside the composition root is a boundary
...
const APPLICATION_INFRASTRUCTURE_ALLOWLIST: Readonly<Record<string, readonly string[]>> = Object.freeze({
  'runtime.ts': ['../infrastructure/hosts.js'],
});

test('application/ (outside mode.ts, the composition root) does not import infrastructure/ beyond the allowlist', () => {
