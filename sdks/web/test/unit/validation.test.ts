/**
 * Pure, offline coverage for the validators (spec/control-points.md
...
import type { ContextPolicy } from '@fireweaveai/web-sdk';
...
test('validateControlPointKey: rejects a key containing a control character', () => {
  ...
  const withDel = validateControlPointKey(`checkout${String.fromCharCode(0x7f)}v2`);
...
test('validateContext: rejects a context above the attribute-count bound', () => {
  ...
  for (let i = 0; i < DEFAULT_CONTEXT_LIMITS.maxAttributeCount + 1; i += 1) attrs[`k${i}`] = i;
...
test('validateContext: rejects an attribute key over the byte bound', () => {
  const result = validateContext({ attributes: { ['k'.repeat(DEFAULT_CONTEXT_LIMITS.maxKeyBytes + 1)]: 1 } }, POLICY);
  assert.equal(result.ok, false);
  assert.ok(!result.ok && result.error.kind === 'InvalidContext');
});

test('validateContext: rejects a reserved attribute key', () => {
  const [reserved] = DEFAULT_RESERVED_ATTRIBUTE_KEYS;
  assert.ok(reserved !== undefined);
  const result = validateContext({ attributes: { [reserved]: 'x' } }, POLICY);
  assert.equal(result.ok, false);
  assert.ok(!result.ok && result.error.kind === 'InvalidContext');
});
...
test('validateContext: extracts groups/groupProperties rather than treating them as ordinary attributes', () => {
  const result = validateContext(
    {
      attributes: {
        'fireweave.groups': { team: 'growth' },
        'fireweave.groupProperties': { team: { tier: 'enterprise' } },
      },
    },
    POLICY
  );
  assert.equal(result.ok, true);
  assert.ok(result.ok && result.value.groups?.['team'] === 'growth');
  assert.ok(result.ok && result.value.groupProperties?.['team']?.['tier'] === 'enterprise');
  assert.equal(result.ok && 'fireweave.groups' in result.value.attributes, false);
});
