 ...
 * nothing objected, with the first evidence otherwise arriving in
 * production.
 *
 * Recording plus an explicit trace keeps that guarantee by a different
 * route: nothing is silent, and local dev can exercise targeting rules
 * offline. These tests pin both halves — the recording AND the trace —
 * because dropping the trace would restore exactly the failure the old
 * design was avoiding.
 */
import { test } from 'node:test';
import assert from 'node:assert/strict';
import { FireweaveLocalWebAdapter, FireweaveWebRuntime, FireweaveWebClient } from '@fireweaveai/web-sdk';
...
test('identify() registers the target AND re-prefetches under its targetingKey', async () => {
  const { adapter } = harness();
  const runtime = new FireweaveWebRuntime(adapter);
  await runtime.initialize();
  const fw = new FireweaveWebClient(runtime);

  const result = await fw.identify('user_5', { properties: { plan: 'pro' } });
  assert.equal(result.ok, true);
  assert.deepEqual(adapter.getRegisteredTargets()[0]?.properties, { plan: 'pro' });
